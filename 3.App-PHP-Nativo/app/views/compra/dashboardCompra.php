<!DOCTYPE html>
<html lang="es">

<body>
  <div class="app">
    <!-- Sidebar -->
    <?php require_once __DIR__ . '/../layouts/sidebar.php'; ?>

    <?php require_once __DIR__ . '/modalConsultas.php';
    ?>
    <main class="main">
      <!-- Topbar -->
      <header>
        <strong>PANADERIA WYK— Panel Control</strong>
        <div class="search">
          <lord-icon
            src="https://cdn.lordicon.com/vhdgmtyj.json"
            trigger="hover"
            stroke="bold"
            colors="primary:#933e0d"
            style="width:50px;height:50px">
          </lord-icon>
          <input id="buscarRapido" placeholder="Buscar…" />
          <button type="button" class="btn btn-ghost" data-bs-toggle="modal" data-bs-target="#filtroAvanzadoModal">
            <i data-feather="sliders" style="width:18px;height:18px;color:#94a3b8;"></i>
          </button>
        </div>
        <a href="<?php echo \config\APP_URL; ?>compras/create" rel="noopener noreferrer"><button type="button" class="btn btn-primary">+ Nueva compra</button></a>
      </header>

      <div class="layout">
        <section class="left">
          <!-- Hero -->
          <div class="hero">
            <div>
              <div class="hero-note">¡Buen día!</div>
              <h2>Bienvenido, <span style="color:var(--primary)"><?php echo $_SESSION['userName']; ?></span></h2>
              <div class="hero-note" style="font-style: 30px;">Que el aroma del pan recién horneado te acompañe hoy 🍞</div>
              <div style="margin-top:12px; display:flex; gap:10px;">
                <button class="btn btn-ghost">Ver agenda</button>
              </div>
            </div>
            <div class="illus">
              <div class="floaty"></div>
              <div class="bread">🥖</div>
              <div class="croissant">🥐</div>
            </div>
          </div>

          <?php if ($_SESSION['rol'] == 'ADMINISTRADOR') {
          ?>
            <!-- KPIs -->
            <section class="stats">
              <div class="stat">
                <div>
                  <div class="muted">Compras existentes</div>
                  <div class="kpi"><?php echo $dashboardDataCompras['comprasExistentes']; ?></div>
                </div>
                <div class="icon">
                  <lord-icon
                    src="https://cdn.lordicon.com/oqhqyeud.json"
                    trigger="hover"
                    stroke="bold"
                    colors="primary:#933e0d,secondary:#933e0d"
                    style="width:40px;height:40px">
                  </lord-icon>
                </div>
              </div>
              <div class="stat">
                <div>
                  <div class="muted">Compras pendientes</div>
                  <div class="kpi"><?php echo $dashboardDataCompras['comprasPendientes']; ?></div>
                </div>
                <div class="icon">
                  <lord-icon
                    src="https://cdn.lordicon.com/oqhqyeud.json"
                    trigger="hover"
                    stroke="bold"
                    colors="primary:#933e0d,secondary:#933e0d"
                    style="width:40px;height:40px">
                  </lord-icon>
                </div>
              </div>
              <div class="stat">
                <div>
                  <div class="muted">Compras pagadas</div>
                  <div class="kpi"><?php echo $dashboardDataCompras['comprasPagadas']; ?></div>
                </div>
                <div class="icon">
                  <lord-icon
                    src="https://cdn.lordicon.com/oqhqyeud.json"
                    trigger="hover"
                    stroke="bold"
                    colors="primary:#933e0d,secondary:#933e0d"
                    style="width:40px;height:40px">
                  </lord-icon>
                </div>
              </div>
              <div class="stat">
                <div>
                  <div class="muted">Compras canceladas</div>
                  <div class="kpi"><?php echo $dashboardDataCompras['comprasCanceladas']; ?></div>
                </div>
                <div class="icon">
                  <lord-icon
                    src="https://cdn.lordicon.com/oqhqyeud.json"
                    trigger="hover"
                    stroke="bold"
                    colors="primary:#933e0d,secondary:#933e0d"
                    style="width:40px;height:40px">
                  </lord-icon>
                </div>
              </div>
            </section><br><br>
          <?php
          }
          ?>

          <!-- Tabla de compras -->
          <div class="section-reportes">
            <table id="tablaCompras" class="tabla-consultas">
              <tbody>
                <div class="table-header">
                  <strong>Tabla de compras</strong>
                  <div style="display:flex; gap:8px;">
                    <button id="generatePdfBtn" style="display:flex; align-items:center;" class="btn btn-ghost btp-personalizado">
                      <span>Generar PDF</span>
                      <lord-icon
                        src="https://cdn.lordicon.com/gyyhoycg.json"
                        trigger="hover"
                        stroke="bold"
                        colors="primary:#fff"
                        style="width:50px;height:50px">
                      </lord-icon>
                    </button>
                  </div>
                </div>
                <thead>
                  <tr>
                    <th>Fecha</th>
                    <th>Tipo</th>
                    <th>Total</th>
                    <th>Proveedor</th>
                    <th>Marca</th>
                    <th>Teléfono</th>
                    <th>Email</th>
                    <th>Descripción</th>
                    <th>Registrado por</th>
                    <th>Estado de compra</th>
                    <th colspan="2" style="text-align:center">Acciones</th>
                  </tr>
                </thead>
                <?php

                if ($dashboardDataCompras['compras']) {
                  foreach ($dashboardDataCompras['compras'] as $compra) {
                ?>
                    <tr>
                      <td> <?php echo htmlspecialchars($compra['FECHA_HORA_COMPRA']); ?></td>
                      <td> <?php echo htmlspecialchars($compra['TIPO']); ?></td>
                      <td> <?php echo htmlspecialchars($compra['TOTAL_COMPRA']); ?></td>
                      <td> <?php echo htmlspecialchars($compra['NOMBRE_PROVEEDOR']); ?></td>
                      <td> <?php echo htmlspecialchars($compra['MARCA']); ?></td>
                      <td> <?php echo htmlspecialchars($compra['TEL_PROVEEDOR']); ?></td>
                      <td> <?php echo htmlspecialchars($compra['EMAIL_PROVEEDOR']); ?></td>
                      <td> <?php echo htmlspecialchars($compra['DESCRIPCION_COMPRA']); ?></td>
                      <td> <?php echo htmlspecialchars($compra['USUARIO_REGISTRO']); ?></td>
                      <td> <?php echo htmlspecialchars($compra['ESTADO_FACTURA_COMPRA']); ?></td>
                      <td>
                        <a href="<?php echo \config\APP_URL . 'compras/viewEdit/' . htmlspecialchars($compra['ID_COMPRA']); ?>" class='btn btn-sm btn-primary btn-actualizar'>
                          <lord-icon
                            src="https://cdn.lordicon.com/ibckyoan.json"
                            trigger="hover"
                            colors="primary:#ffffff"
                            style="width:30px;height:30px">
                          </lord-icon>
                        </a>
                      </td>
                      <td>
                        <button
                          type="button"
                          class='btn btn-sm btn-primary btn-detalle-compra'
                          data-bs-toggle="modal"
                          data-bs-target="#detalleCompraModal"
                          data-id-compra="<?= htmlspecialchars($compra['ID_COMPRA']); ?>">
                          Detalle
                        </button>
                      </td>
                    </tr>
                  <?php
                  }
                } else {
                  ?>
                  <tr>
                    <td colspan='8' style="text-align:center;">No hay registros de compras disponibles.</td>
                  </tr>
                <?php
                }
                ?>
              </tbody>
            </table>
          </div>
      </div>
      <?php require_once __DIR__ . '/../layouts/footers/footerDashboard.php'; ?>
    </main>

    <div class="modal fade" id="detalleCompraModal" tabindex="-1" aria-labelledby="detalleCompraModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="detalleCompraModalLabel">Detalle de Compra</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>

          <div class="modal-body">
            <table class="table table-bordered">
              <thead>
                <tr>
                  <th>Tipo</th>
                  <th>Item</th>
                  <th>Cantidad</th>
                  <th>Precio Unitario</th>
                  <th>Subtotal</th>
                </tr>
              </thead>
              <tbody id="detalleCompraBody">
              </tbody>
            </table>
            <div class="text-end">
              <strong>Total General:</strong>
              <span id="compraTotalDisplay"></span>
            </div>
          </div>

          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
          </div>
        </div>
      </div>
    </div>

    <!-- LIBRERIAS -->
    <script src="<?php echo \config\APP_URL; ?>public/js/sweetalert2.all.min.js"></script>
    <script src="https://unpkg.com/feather-icons"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
    <script src="https://cdn.lordicon.com/lordicon.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/chart.js"
      integrity="sha256-Lye89HGy1p3XhJT24hcvsoRw64Q4IOL5a7hdOflhjTA="
      crossorigin="anonymous">
    </script>

    <!-- JS Toads y var global -->
    <script>
      const APP_URL = "<?php echo \config\APP_URL; ?>";

      const successMessage = "<?php echo $success_message; ?>";
      const errorMessage = "<?php echo $error_message; ?>";
    </script>
    <script src="<?php echo \config\APP_URL; ?>public/js/toads-sweetalert2.js"></script>

    <!-- JS para CRUD -->
    <script src="<?php echo \config\APP_URL; ?>public/js/dashboard.js"></script>
    <script src="<?php echo \config\APP_URL; ?>public/js/sidebar.js"></script>
    <script src="<?php echo \config\APP_URL; ?>public/js/compra/selectDetalleCompraModal.js"></script>

    <!-- JS para busquedas personalizadas (y PDF) -->
    <script src="<?php echo \config\APP_URL; ?>public/js/compra/busquedaFiltro.js"></script>
    <script src="<?php echo \config\APP_URL; ?>public/js/compra/PDFgenerateFilter.js"></script>
</body>

</html>