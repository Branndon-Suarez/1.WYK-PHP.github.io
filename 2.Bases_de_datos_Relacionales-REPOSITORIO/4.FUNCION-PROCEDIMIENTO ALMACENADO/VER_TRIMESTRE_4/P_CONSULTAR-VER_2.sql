...
CREATE PROCEDURE CONSULTAR_VENTA_MESERO(IN P_C_ID_USUARIO_FK_VENTA INT)
BEGIN
    SELECT 
        V.ID_VENTA,
        V.FECHA_HORA_VENTA,
    ...
    FROM VENTA V
    INNER JOIN USUARIO U ON V.ID_USUARIO_FK_VENTA = U.ID_USUARIO
    WHERE V.ID_USUARIO_FK_VENTA = P_C_ID_USUARIO_FK_VENTA
    ORDER BY V.ID_VENTA;
END $

CREATE PROCEDURE CONSULTAR_VENTA_CAJERO()
BEGIN
    SELECT 
		V.ID_VENTA,
        V.FECHA_HORA_VENTA,
...
BEGIN
    ...
    ORDER BY D.ID_DET_COMPRA_MAT_PRIM;
END $
...
BEGIN
    SELECT
        ...
        D.ID_PRODUCCION_FK_DET_PRODUC,
        PR.NOMBRE_PRODUCCION,
        D.ID_MATERIA_PRIMA_FK_DET_PRODUC,
        M.NOMBRE_MATERIA_PRIMA,
        D.CANTIDAD_REQUERIDA
    FROM DETALLE_PRODUCCION D
    INNER JOIN PRODUCCION PR ON D.ID_PRODUCCION_FK_DET_PRODUC = PR.ID_PRODUCCION
    INNER JOIN MATERIA_PRIMA M ON D.ID_MATERIA_PRIMA = M.ID_MATERIA_PRIMA_FK_DET_PRODUC
    ORDER BY D.ID_DETALLE_PRODUCCION;
END $
